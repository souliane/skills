...
def _best_underline(
    f: FieldRect,
    col_uls: list[tuple[int, Underline]],
    matched: set[int],
    tolerance: float,
) -> tuple[int | None, Underline | None]:
    best_ui: int | None = None
    best_u: Underline | None = None
    best_dist = 999.0
    for ui, u in col_uls:
        if ui in matched:
            continue
        if f.y1 - tolerance <= u.y <= f.y2 + tolerance:
            ...
            if dist < best_dist:
                best_dist, best_ui, best_u = dist, ui, u

    if best_u is None:
        available = [(ui, u) for ui, u in col_uls if ui not in matched]
        if available:
            best_ui, best_u = min(available, key=lambda p: abs(p[1].y - (f.y1 + 1.5)))
    return best_ui, best_u


@dataclass
class MatchState:
    matched_uls: set[int] = field(default_factory=set)
    matched_fs: set[int] = field(default_factory=set)
...
def _match_column(
    ...
    col_fields = sorted(((fi, f) for fi, f in enumerate(fields) if f.column == col), key=lambda p: -p[1].y1)
    col_uls = sorted(((ui, u) for ui, u in enumerate(underlines) if u.column == col), key=lambda p: -p[1].y)

    results: list[AlignmentResult] = []
    for fi, f in col_fields:
        best_ui, best_u = _best_underline(f, col_uls, state.matched_uls, tolerance)
        if best_u is None:
            results.append(AlignmentResult(field=f, nearest_underline=None, offset=999, aligned=False))
            continue

        offset = best_u.y - f.y1
        aligned = f.y1 - tolerance <= best_u.y <= f.y2 + tolerance
        result = AlignmentResult(field=f, nearest_underline=best_u, offset=offset, aligned=aligned)
        if aligned:
            state.matched_uls.add(best_ui)
            state.matched_fs.add(fi)
        else:
            suggested_y1 = best_u.y - 1.5
            result.suggested_rect = (suggested_y1, suggested_y1 + f.height)
...
def match_fields_to_underlines(
    ...
    results: list[AlignmentResult] = []
    for col in (1, 2):
        results.extend(_match_column(col, fields, underlines, tolerance, state))

    unmatched_uls = [u for ui, u in enumerate(underlines) if ui not in state.matched_uls]
    unmatched_fs = [f for fi, f in enumerate(fields) if fi not in state.matched_fs]
...
class PageRaster:
    ...
    def clamp_x(self, value: int) -> int:
        return max(0, min(value, self.width - 1))

    def clamp_y(self, value: int) -> int:
        return max(0, min(value, self.height - 1))


def _load_page_raster(golden_pdf: str | Path, page: int, png_path: Path) -> PageRaster:
    ...
    mediabox = pdf.pages[page - 1].get("/MediaBox")
    if mediabox:
    ...
    return PageRaster(img, img_w, img_h, pdf_h, img_w / pdf_w, img_h / pdf_h)


def _check_field_coverage(f: FieldRect, raster: PageRaster) -> tuple[str, bool, str]:
    px1 = raster.clamp_x(int(f.x1 * raster.scale_x))
    px2 = raster.clamp_x(int(f.x2 * raster.scale_x))
    py1 = raster.clamp_y(int((raster.pdf_h - f.y2) * raster.scale_y))
    py2 = raster.clamp_y(int((raster.pdf_h - f.y1) * raster.scale_y))

    if px2 <= px1 or py2 <= py1:
        return (f.name, False, "invalid pixel region")

    pixels = list(raster.img.crop((px1, py1, px2, py2)).getdata())
    non_white = sum(1 for r, g, b in pixels if r < 240 or g < 240 or b < 240)
    coverage = non_white / len(pixels) if pixels else 0
    if coverage < 0.01:
        return (f.name, False, f"no AP visible (coverage={coverage:.1%})")
    return (f.name, True, f"coverage={coverage:.1%}")


def _check_underline_coverage(u: Underline, raster: PageRaster) -> tuple[str, bool, str] | None:
    px_start = raster.clamp_x(int(u.x * raster.scale_x))
    px_end = raster.clamp_x(int((u.x + u.width * u.scale) * raster.scale_x))
    py_center = int((raster.pdf_h - u.y) * raster.scale_y)
    py1 = max(0, py_center - 3)
    py2 = min(raster.height - 1, py_center + 3)

    if px_end <= px_start or py2 <= py1:
        return None

    pixels = list(raster.img.crop((px_start, py1, px_end, py2)).getdata())
    grey_count = sum(1 for r, g, b in pixels if 150 < r < 230 and 150 < g < 230 and 150 < b < 230)
    if grey_count < 5:
        return (f"underline@y={u.y:.1f}({u.col_label})", False, "no grey pixels found at underline position")
    return None
...
def check_pixel_coverage(
    ...
    with tempfile.TemporaryDirectory(prefix="verify-pixel-") as tmpdir:
        ...
        raster = _load_page_raster(golden_pdf, page, png_path)
        results = [_check_field_coverage(f, raster) for f in fields]
        results.extend(r for u in underlines if (r := _check_underline_coverage(u, raster)) is not None)
        return results
...
def _header_lines(report: TemplateReport) -> list[str]:
    n_ul1, n_ul2 = len(report.underlines_col1), len(report.underlines_col2)
    n_f1, n_f2 = len(report.fields_col1), len(report.fields_col2)
    status_1 = "OK" if n_ul1 == n_f1 else "MISMATCH"
    status_2 = "OK" if n_ul2 == n_f2 else "MISMATCH"
    return [
        f"\n{'=' * 70}",
        f"  {report.name}",
        f"  {report.path}",
        f"{'=' * 70}",
        f"\n  Page {report.page}:",
        f"    Col1: {n_ul1} underlines, {n_f1} fields -- {status_1}",
        f"    Col2: {n_ul2} underlines, {n_f2} fields -- {status_2}",
    ]


def _verbose_lines(report: TemplateReport) -> list[str]:
    lines: list[str] = []
    for a in report.alignments:
        if a.nearest_underline:
            ...
            lines.append(
                f"    {a.field.name}:  rect_y=[{a.field.y1:.1f}, {a.field.y2:.1f}]"
                f"  underline_y={a.nearest_underline.y:.1f}  offset={a.offset:+.1f}  {status}"
            )
        else:
...
def _misaligned_lines(report: TemplateReport) -> list[str]:
    ...
    for a in misaligned:
        if a.nearest_underline:
            lines.append(
                f"    {a.field.name}:  rect_y=[{a.field.y1:.1f}, {a.field.y2:.1f}]"
                f"  nearest_underline_y={a.nearest_underline.y:.1f}  offset={a.offset:+.1f}"
            )
            if a.suggested_rect:
...
def _pixel_lines(report: TemplateReport) -> list[str]:
    if not report.pixel_results:
        return []
    ok_count = sum(1 for _, ok, _ in report.pixel_results if ok)
    lines = [f"\n  [Pixel check] {ok_count}/{len(report.pixel_results)} OK"]
    lines.extend(f"    FAIL: {name} -- {detail}" for name, ok, detail in report.pixel_results if not ok)
    return lines


def format_report(report: TemplateReport, *, verbose: bool = False) -> str:
    """Format a template report for console output."""
    aligned_count = sum(1 for a in report.alignments if a.aligned)
    lines = _header_lines(report)
    lines.append(f"\n  Field alignment: {aligned_count}/{len(report.alignments)} OK")

    if verbose:
        lines.extend(_verbose_lines(report))
    lines.extend(_misaligned_lines(report))
    ...
    lines.extend(_pixel_lines(report))
...
def _section_y_range(section: str | None) -> tuple[float, float]:
    if not section:
        return 5.0, 300.0
    if section == "income":
        return 50.0, 280.0
...
def verify_template(pdf_path: str, config: VerifyConfig) -> TemplateReport:
    """Run all verification checks on a single template."""
    name = Path(pdf_path).stem
    report = TemplateReport(path=pdf_path, name=name, page=config.page)
    ...
    if config.page < 1 or config.page > len(pdf.pages):
        report.errors.append(f"Page {config.page} out of range (PDF has {len(pdf.pages)} pages)")
        pdf.close()
        return report

    pg = pdf.pages[config.page - 1]
    y_min, y_max = _section_y_range(config.section)
    ...
    results, unmatched_uls, unmatched_fs = match_fields_to_underlines(
        all_fields, all_underlines, tolerance=config.tolerance
    )
    ...
    if config.pixel and config.golden_dir:
        golden_pdf = _find_golden_pdf(pdf_path, config.golden_dir)
        if golden_pdf:
            report.pixel_results = check_pixel_coverage(golden_pdf, config.page, all_fields, all_underlines)
        else:
            report.errors.append(f"No golden PDF found in {config.golden_dir} for {name}")
...
def _run_templates(pdf_paths: list[str], config: VerifyConfig, *, fix: bool, verbose: bool) -> list[TemplateReport]:
    reports: list[TemplateReport] = []
    for pdf_path in pdf_paths:
        report = verify_template(pdf_path, config)
...
def _print_cross_consistency(reports: list[TemplateReport]) -> bool:
    print(f"\n{'=' * 70}")
...
def _print_summary(reports: list[TemplateReport], *, all_ok: bool) -> None:
