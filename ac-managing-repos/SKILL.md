...
4. **Squash integrity check.** Before any rewrite, save the tip: `OLD_TIP=$(git rev-parse HEAD)`. After rewrite, verify `git diff $OLD_TIP..HEAD` is empty. If there is any diff, the rewrite lost or introduced changes — abort and investigate.
