...
4. **Open-PR sweep (Non-Negotiable).** Before scoping the review, list every open PR/MR across all managed repos (e.g. `gh pr list --json number,title,mergeable,mergeStateStatus,statusCheckRollup,reviewDecision`). For each one, summarize its mergeability, CI status, and review state, then ask the user whether to merge it before the review starts. **The review must run against the latest state.** Outstanding PRs that are about to land would otherwise produce stale findings and force a re-review. If a PR is `BEHIND` its base, update it first; if CI is red, surface the failing job(s) and fix the root cause — never bypass with `--no-verify`. Skip a PR only with explicit user confirmation, and surface it in the final report so the user knows the review excluded it.
