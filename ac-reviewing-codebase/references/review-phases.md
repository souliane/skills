...
- **Comments as code — one line, or none (report, never gate).** Every comment the change ADDS is one line carrying a non-obvious *why*. A multi-line block narrating what the code below already says is a finding; the fix is a rename or a split, not shorter prose. Legitimate exceptions (threat model, protocol quirk, measured constant) are judged, not banned. Grep the diff for runs of 3+ consecutive comment lines and for docstrings that merely echo the signature. Judge only what the change adds — rewriting pre-existing comments is unrelated churn. Never propose a lint rule or CI gate on comment length; advisory warnings are the ceiling (see SKILL.md Rule 16).
