...
class TestCountLintViolations:
    ...
    def test_clean_dir(self, tmp_path: Path) -> None:
        ...
        result = cli._count_lint_violations(tmp_path)
        # May have violations or not depending on ruff config, but should have structure
...
class TestCheckCoverage:
    ...
    def test_with_invalid_coverage(self, tmp_path: Path) -> None:
        (tmp_path / ".coverage").write_text("not valid", encoding="utf-8")
        result = cli._check_coverage(tmp_path)
        # coverage json will fail on invalid data
