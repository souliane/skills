...
If `thinkingDefault` is missing from `openclaw.json`, sessions default to `"off"`. On a reasoning-required model, every call then fails with `400 Reasoning is mandatory for this endpoint and cannot be disabled` — errors that are swallowed silently, so the user only sees degraded output (hallucinated URLs, tag leaks, short bland replies). Set `agents.defaults.thinkingDefault: "medium"` when the primary model is reasoning-required.

**Important — prefer `"medium"` over `"high"` for press-review and other long-synthesis tasks.** `"high"` causes the model to burn its entire per-response `max_tokens` budget on internal reasoning before producing any visible output (the trace shows `stopReason: "length"`, `thinking_chars > 16000, text_chars == 0`). At `"medium"` the same 120-item press-review synthesis uses ~3800 output tokens and completes cleanly. If you need high effort for a specific chat task, use the per-task override pattern (see `troubleshooting-and-maintenance.md` § Per-Task Reasoning Overrides). Verify with `jq -r 'select(.type=="thinking_level_change") | .thinkingLevel' <session>.jsonl` — should be `"medium"`, not `"off"` and not `"high"`.

### `maxTokens` gotcha — declare the model explicitly

If the primary model is an OpenRouter model that is NOT listed in `agents/<id>/agent/models.json`, OpenClaw resolves its metadata dynamically but falls back to `DEFAULT_MODEL_MAX_TOKENS = 8192` for the per-request `max_tokens`. On a reasoning model at medium/high thinking, 8192 is often not enough — the response truncates silently and the user receives the `⚠️ Agent couldn't generate a response` apology instead of the press review. **Add the explicit model entry to every agent that uses it**, with `maxTokens` matching the model's real cap. For `openai/gpt-oss-120b`, use `maxTokens: 65536`. Verify: `jq '.providers.openrouter.models[] | select(.id == "openai/gpt-oss-120b") | .maxTokens' ~/.openclaw/agents/<agent>/agent/models.json` — should return a number, not null.
